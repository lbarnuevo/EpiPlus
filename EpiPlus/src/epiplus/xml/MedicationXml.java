...
public class MedicationXml {
	...
	public static void medication2Xml(Medication medication) throws JAXBException {

		JAXBContext jaxbContext = JAXBContext.newInstance(Medication.class);
		// Get the marshaller
		...
		File file = new File("./xmls/External-Medication.xml");

		marshaller.marshal(medication, file);

	}

	// Create .java file from xml file

	public static Medication xml2Medication() throws JAXBException {
		...
		File file = new File("./xmls/External-Medication.xml");
		Medication medication = (Medication) unmarshaller.unmarshal(file);
